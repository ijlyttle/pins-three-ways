...
This file, `pins.txt` is written to the board's root directory; it will make it easier to create a `board_url()` to read pins.
...
This uses the manifest file, `pins.txt`, to compile the information needed to build a `pins::board_url()`.
